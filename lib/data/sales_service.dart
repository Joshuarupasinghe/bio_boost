...
class SalesService {
  ...
  Future<String?> addSale(Sales sale) async {
    try {
      DocumentReference docRef = await _firestore.collection(_collectionPath).add(sale.toMap());
      return docRef.id;
    } catch (e) {
  ...
  Future<bool> updateSale(String documentId, Map<String, dynamic> saleData) async {
    try {
      await _firestore.collection(_collectionPath).doc(documentId).update(saleData);
      return true;
    } catch (e) {
